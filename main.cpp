...
int main(int argc, char* argv[]) {
    ...
    Trie trie = Trie();
    for (int i = 0; i < numberOfGames; i++) trie.insert(&games[i]);
    
    std::vector<Game*> result = trie.autocomplete(prefix, k);

    if (result.size() == 0) std::cout << "No results found" << std::endl;
    else for (Game* game: result) std::cout << game->getTitle() << " | " << game->getShortDescription() << " | " << game->getPopularity() << std::endl;
}